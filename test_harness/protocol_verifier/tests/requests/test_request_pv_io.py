...
def timeout(delay: float) -> Callable[..., Coroutine[Any, Any, None]]:
...
async def test_get_request_json_response_timeout_error() -> None:
    ...
    with patch(
        "aiohttp.connector.BaseConnector.connect", side_effect=timeout(1)
    ):
        with pytest.raises(TimeoutError):
            await get_request_json_response(url=url, read_timeout=0.1)
...
async def test_get_request_json_response_timeout_no_error() -> None:
    ...
    with patch(
        ...
        with pytest.raises(RuntimeError) as error:
            await get_request_json_response(url=url, read_timeout=2)
