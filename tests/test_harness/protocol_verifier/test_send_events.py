...
class TestPVResponseConverter:
    ...
    def test_convert(job_list: list[dict[str, str | list[str]]]) -> None:
        ...
        job_info = {"job_info": "job_info"}
        responses = ["", "Bad"]
        (list_dict, file_name, out_job_id, out_job_info, result, time_completed) = (
        ...
        assert bool(uuid4hex.match(file_name.replace("-", "").replace(".json", "")))
...
class TestPVMessageExceptionHandler:
    ...
    def test_handle_exception_kafka(
        ...
        for exception_class, error_message in [
            ...
            assert f"Error sending payload to kafka: {exception}" in caplog.text
...
class TestPVMessageSender:
    ...
    async def test_send_aio_kafka(
        job_list: list[dict[str, str | list[str]]], kafka_producer_mock: list[str]
    ) -> None:
        ...
        assert bool(uuid4hex.match(results[1].replace("-", "").replace(".json", "")))
    ...
    async def test_send_aio_kafka_timeout_error(
        ...
        monkeypatch.setattr(aiokafka.AIOKafkaProducer, "send_and_wait", mock_send_wait)
        message_bus_kwargs = dict(
            bootstrap_servers="localhost:9092",
        )
        producer_kwargs = dict(
    ...
    async def test_send_sync_kafka(
        job_list: list[dict[str, str | list[str]]], sync_kafka_producer_mock: list[str]
    ) -> None:
        ...
        producer_kwargs = dict(
            topic="test_topic",
            response_converter=PVMessageResponseConverter(message_bus="KAFKA3"),
            exception_converter=PVMessageExceptionHandler(message_bus="KAFKA3"),
        )
        async with get_producer_context(
            message_bus="KAFKA3",
            message_bus_kwargs=message_bus_kwargs,
        ...
        assert all(action == "send" for action in sync_kafka_producer_mock[1:-1])
    ...
    async def test_send_http(
        ...
        with aioresponses.aioresponses() as mock:
            mock.post(
                "http://localhost:8080/sendevents", status=200, body="test response"
            )
            async with get_producer_context(
                message_bus="HTTP",
                message_bus_kwargs=message_bus_kwargs,
    ...
    async def test_send_http_timeout_error(
        ...
        with aioresponses.aioresponses() as mock:
            mock.post(
                "http://localhost:8080/sendevents", exception=asyncio.TimeoutError()
            )
            async with get_producer_context(
                message_bus="HTTP",
                message_bus_kwargs=message_bus_kwargs,
...
def test_get_message_bus_kwargs() -> None:
    ...
    check_dict_equivalency(dict(bootstrap_servers="localhost:9092"), message_bus_kwargs)
...
def test_get_producer_kwargs():
    ...
    for details in [
        dict(
            ...
            response_conv=(PVMessageResponseConverter._kafka_conversion_function),
        ...
        dict(
            ...
            response_conv=(PVMessageResponseConverter._http_conversion_function),
        ...
        assert producer_kwargs[details["location"][0]] == (details["location"][1])
        assert isinstance(
        ...
        assert (producer_kwargs["response_converter"].data_conversion_function) == (
